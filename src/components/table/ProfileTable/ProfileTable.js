...
const ListingTable = () => {
  ...
  const getCollections = async () => {
    try {
      ...
      if (response.status) {
        let datas = response.result.filter((val) => val.walletAddress === wallet.address);
        let result = datas.map((val) => val._id);
        getActivity(result);
      }
    }
    catch (e) {
  ...
  const getActivity = async (res) => {
    try {
      let url = "viewActivities";
      let response = await getMethod({ url });
      if (response.status) {
        let profilesactivity = [];
        for(let i=0;i<res.length;i++){
          const datas = response.result.filter((val) => val.collectionId === res[i]);
          profilesactivity.push(...datas);
        }
        setActivityData(profilesactivity);
      }
    }
    catch (e) {
      console.log("error in getactivity", e);
    }
  };
            ...
            <TableContainer className="pricetableinnftdetails mt-4">
              <Table stickyHeader aria-label="sticky table">
                ...
                <TableBody sx={{ paddingTop: "5%" }} className="ntffooterdtbleindetails">
                  {activityData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, i) => {
                    return (<>
